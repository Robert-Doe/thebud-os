...
uint32_t process_exit(uint32_t current_esp) {
    ...
    /* Mark the calling process dead.  Its stack slot is now orphaned — a real
     * OS would free it here, but kfree from interrupt context needs care.
     * For this teaching OS we leave the stack in place (4 KB "leak" per exit). */
    current->esp   = current_esp;
    ...
    while (next->state != PROC_READY) {
        if (next == current) {
            /* Every other process is also dead — fall back to the idle process.
             * In a complete OS the idle process is always READY, but guard here
             * to prevent an infinite loop on a logic bug. */
