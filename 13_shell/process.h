...
/*
 * process_exit(current_esp) — terminate the calling process.
 * Marks the current process DEAD, advances to the next READY process, and
 * returns its saved ESP so isr_common_stub can switch to it immediately.
 * Called from the SYS_EXIT syscall handler inside interrupt_handler().
