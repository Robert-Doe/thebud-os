...
/* Register the syscall dispatch handler for int 0x80.
 * fn(frame) reads EAX (syscall number) and EBX/ECX/EDX (args) from the frame,
 * writes the return value into frame->eax, and returns the ESP to restore
 * (same frame for most syscalls; a different process's ESP for SYS_EXIT). */
