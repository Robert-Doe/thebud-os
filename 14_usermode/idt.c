...
extern void isr128(void);  /* syscall gate — int 0x80 */
...
void idt_init(void) {
    ...
    /* Vector 0x80 — software syscall gate.
     * DPL=3 so ring-3 code can call `int 0x80` without a #GP.
     * The handler runs in ring 0 (GDT_SEL_CODE is a ring-0 segment). */
    idt_set_gate(0x80, (uint32_t)isr128, GDT_SEL_CODE, IDT_GATE_SYSCALL);
