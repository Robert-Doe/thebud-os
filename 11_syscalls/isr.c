...
uint32_t interrupt_handler(struct interrupt_frame *frame) {
    ...
    /* Vector 0x80 — software syscall.  The handler stores its return value
     * in frame->eax so the caller sees it in EAX after `int 0x80` returns.
     * It may also return a different ESP (e.g. SYS_EXIT switches processes). */
    if (vec == 0x80) {
        if (syscall_hook != 0) {
            return syscall_hook(frame);
        }
