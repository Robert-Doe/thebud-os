    ...
    for (pdi = 0; pdi < 1024; pdi++) {
        ...
        if (pdi == 0) {
            ...
            for (pti = 0; pti < 1024; pti++) {
                ...
                uint32_t new_frame = pmm_alloc_page();
                uint32_t pte_flags = src_pt[pti] & 0xFFFu;
                if (!new_frame) return 0;

                copy_4k((uint32_t *)new_frame, (uint32_t *)src_frame);
...
void paging_free_address_space(uint32_t cr3) {
    ...
    for (pdi = 0; pdi < 1024; pdi++) {
        ...
        if (pdi == 0) {
            ...
            /* User region: free every present physical frame, then the PT. */
            for (pti = 0; pti < 1024; pti++) {
                if (pt[pti] & PAGE_PRESENT)
                    pmm_free_page(pt[pti] & ~0xFFFu);
            }
            pmm_free_page((uint32_t)pt);
        }
    }
