...
struct process {
    ...
    struct file_descriptor fd_table[MAX_FDS]; /* open file descriptors           */
    struct vm_region       vm_regions[MAX_REGIONS]; /* demand-paged anon regions */
    int                    is_thread;         /* 1 if secondary thread           */
    uint32_t               thread_group_id;   /* PID of group leader             */
    int                    priority;          /* SCHED_PRIO_* — set at creation  */
};
...
/* ── VFS fd table accessor ──────────────────────────────────────────────── */

struct file_descriptor *vfs_get_fd_table(void);

/* ── VM region table accessor ───────────────────────────────────────────── */

struct vm_region *process_get_vm_regions(void);

/* ── CR3 accessor for shared memory mapping ─────────────────────────────── */

uint32_t process_get_cr3(void);
