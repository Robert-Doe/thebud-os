...
#define SYS_SETPRIORITY   19
#define SYS_GETPRIORITY   20
#define SYS_SOCKET        21   /* udp_bind(port) -> sock_id */
#define SYS_SENDTO        22   /* udp_send(dst_ip, src_port, dst_port, buf, len) */
#define SYS_RECVFROM      23   /* udp_recvfrom(sock_id, buf, buflen, &src_ip, &src_port) */
...
static inline int      sys_shmget(uint32_t key, uint32_t size)          { return syscall(SYS_SHMGET, (int)key, (int)size, 0); }
static inline uint32_t sys_shmat(int shm_id, uint32_t vaddr)            { return (uint32_t)syscall(SYS_SHMAT, shm_id, (int)vaddr, 0); }
static inline int      sys_thread_create(void (*fn)(void), void *stack)  { return syscall(SYS_THREAD_CREATE, (int)fn, (int)stack, 0); }
static inline void     sys_thread_exit(void)                             { syscall(SYS_THREAD_EXIT, 0, 0, 0); }
