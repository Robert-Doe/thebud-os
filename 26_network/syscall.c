...
static int do_pipe(int *fds) {
    if (!fds) return SYSCALL_EINVAL;
    return pipe_alloc(fds);
}

/* ── SYS_MMAP ──────────────────────────────────────────────────────────── */

/* Next available mmap base -- starts above user stack (0x800000) */
static uint32_t next_mmap_base = 0xC00000u;

static uint32_t do_mmap(uint32_t hint, uint32_t len, int prot) {
    ...
    /* Use next_mmap_base if hint is 0 */
    if (hint == 0) {
        base = next_mmap_base;
        next_mmap_base += (len + 0xFFFu) & ~0xFFFu;
    } else {
        base = hint & ~0xFFFu;
    }

    /* Access the current process's vm_regions via process API */
    regions = process_get_vm_regions();
    if (!regions) return 0;

    if (!mmap_reserve(regions, base, len, prot)) return 0;
    return base;
}
...
uint32_t syscall_dispatch(struct interrupt_frame *frame) {
    ...
    switch (num) {
        ...
        case SYS_PIPE:
            frame->eax = (uint32_t)do_pipe((int *)a);
            break;

        case SYS_MMAP:
            frame->eax = do_mmap((uint32_t)a, (uint32_t)b, (int)frame->edx);
            break;

        case SYS_SHMGET:
            frame->eax = (uint32_t)shm_get((uint32_t)a, (uint32_t)b);
            break;

        case SYS_SHMAT:
            frame->eax = shm_at(a, (uint32_t)b);
            break;

        case SYS_THREAD_CREATE:
            frame->eax = process_thread_create((uint32_t)a, (uint32_t)b);
            break;

        case SYS_THREAD_EXIT:
            return do_exit((uint32_t)frame, 0);
