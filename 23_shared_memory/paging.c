...
void paging_set_user_page(uint32_t cr3, uint32_t virt_addr) {
    ...
    /* Allocate a new PT if the PDE is not present */
    if (!(pd[pdi] & PAGE_PRESENT)) {
        pt = (uint32_t *)pmm_alloc_page();
        if (!pt) return;
        zero_page(pt);
        pd[pdi] = (uint32_t)pt | PAGE_PRESENT | PAGE_WRITABLE | PAGE_USER;
    } else {
        pd[pdi] |= PAGE_USER;
    ...
    if (!(pt[pti] & PAGE_PRESENT)) {
        /* Allocate a fresh frame for the demand-paged virtual address */
        new_frame = (uint32_t)pmm_alloc_page();
        if (!new_frame) return;
        zero_page((uint32_t *)new_frame);
        pt[pti] = new_frame | PAGE_PRESENT | PAGE_WRITABLE | PAGE_USER;
    } else {
        pt[pti] |= PAGE_USER;
    }
...
void paging_map_user_phys(uint32_t cr3, uint32_t virt, uint32_t phys) {
    ...
    pt[pti] = phys | PAGE_PRESENT | PAGE_WRITABLE | PAGE_USER;
