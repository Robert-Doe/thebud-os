 ...
 *
 * Mirrors the GDT implementation in gdt.c almost exactly:
 *   - Static array of struct idt_entry (256 × 8 bytes)
 *   - idt_set_gate() fills one entry from readable parameters
 *   - idt_load() calls 'lidt' via inline assembly
 *   - idt_init() installs all 48 handler gates and loads the IDT
 */
...
/* The IDT and its descriptor — private to this file */
static struct idt_entry     idt[IDT_NUM_ENTRIES];
static struct idt_descriptor idtr;

/* -------------------------------------------------------------------------
 * Declare every assembly stub exported from isr.asm.
 * idt_init() takes their addresses and stores them in the IDT gates.
 * ------------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------
 * idt_load — tell the CPU where the IDT lives.
 * 'lidt' is the IDT equivalent of 'lgdt'.
 * No segment-register reload is needed: the CPU uses the IDTR transparently,
 * not through a selector the way code/data segments work.
 * ------------------------------------------------------------------------- */
 ...
 * ------------------------------------------------------------------------- */
void idt_init(void) {
    /* --- CPU exception gates (vectors 0-31) --- */
    ...
    /* --- Hardware IRQ gates (vectors 32-47, after PIC remapping) --- */
    ...
    /* Vectors 48-255: left as zero (null gate) — not expected to fire */
