...
void process_init(void) {
    ...
    for (i = 0; i < MAX_PROCESSES; i++) {
        ...
        for (s = 0; s < MAX_FDS; s++) proc_table[i].fd_table[s].valid = 0;
        mmap_init_table(proc_table[i].vm_regions);
        proc_table[i].is_thread        = 0;
        proc_table[i].thread_group_id  = 0;
    }
    proc_table[0].pid              = next_pid++;
    proc_table[0].state            = PROC_RUNNING;
    proc_table[0].next             = &proc_table[0];
    proc_table[0].cr3              = paging_kernel_cr3();
    proc_table[0].is_thread        = 0;
    proc_table[0].thread_group_id  = proc_table[0].pid;
    current    = &proc_table[0];
    proc_count = 1;
    /* Pre-open stdin(0), stdout(1), stderr(2) for idle process */
    proc_table[0].fd_table[0].valid = 1; proc_table[0].fd_table[0].inode = 0; proc_table[0].fd_table[0].offset = 0; proc_table[0].fd_table[0].ops = &vga_ops;
    proc_table[0].fd_table[1].valid = 1; proc_table[0].fd_table[1].inode = 1; proc_table[0].fd_table[1].offset = 0; proc_table[0].fd_table[1].ops = &vga_ops;
    proc_table[0].fd_table[2].valid = 1; proc_table[0].fd_table[2].inode = 2; proc_table[0].fd_table[2].offset = 0; proc_table[0].fd_table[2].ops = &vga_ops;
...
uint32_t process_create(void (*entry)(void)) {
    ...
    for (s = 0; s < MAX_FDS; s++) p->fd_table[s].valid = 0;
    p->fd_table[0].valid = 1; p->fd_table[0].inode = 0; p->fd_table[0].offset = 0; p->fd_table[0].ops = &vga_ops;
    p->fd_table[1].valid = 1; p->fd_table[1].inode = 1; p->fd_table[1].offset = 0; p->fd_table[1].ops = &vga_ops;
    p->fd_table[2].valid = 1; p->fd_table[2].inode = 2; p->fd_table[2].offset = 0; p->fd_table[2].ops = &vga_ops;
    mmap_init_table(p->vm_regions);
    p->is_thread       = 0;
    p->thread_group_id = p->pid;
...
uint32_t process_fork(struct interrupt_frame *frame) {
    ...
    /* Copy fd table, vm_regions, and thread info from parent */
    for (s = 0; s < MAX_FDS; s++) child->fd_table[s] = current->fd_table[s];
    for (s = 0; s < MAX_REGIONS; s++) child->vm_regions[s] = current->vm_regions[s];
    child->is_thread       = 0;
    child->thread_group_id = child->pid;  /* fork creates a new group leader */
...
uint32_t process_exit(uint32_t current_esp) {
    ...
    int is_thread_exit = dead->is_thread;
    int i;

    dead->esp = current_esp;

    if (is_thread_exit) {
        /* Thread: just mark dead, don't zombify (parent won't wait for us) */
        dead->state = PROC_DEAD;
        dead->pid   = 0;
        proc_count--;
    } else {
    ...
    /* Only free address space for non-threads (threads share CR3 with group leader) */
    if (!is_thread_exit && old_cr3 != paging_kernel_cr3())
        paging_free_address_space(old_cr3);
...
uint32_t process_fault_handler(uint32_t esp) {
    ...
    /* Check if fault address is in a demand-paged region */
    {
        int prot = mmap_find(current->vm_regions, fault_addr);
        if (prot >= 0) {
            /* Allocate and map the page on first access */
            paging_set_user_page(current->cr3, fault_addr & ~0xFFFu);
            return esp;  /* resume -- page is now mapped */
        }
    }

    /* Check for CoW write fault: error bit 1 set, user address */
    if ((f->err_code & 2u) && fault_addr >= 0x400000u && fault_addr < 0x800000u) {
        if (paging_cow_fault(current->cr3, fault_addr) == 0)
            return esp;  /* resolved -- resume */
    }
...
struct file_descriptor *vfs_get_fd_table(void) { return current ? current->fd_table : 0; }
struct vm_region *process_get_vm_regions(void) { return current ? current->vm_regions : 0; }
uint32_t process_get_cr3(void)                 { return current ? current->cr3 : 0; }

/* ── process_thread_create ───────────────────────────────────────────── */

uint32_t process_thread_create(uint32_t entry, uint32_t user_stack_top) {
    struct process *t;
    uint8_t *kstack;
    uint32_t *sp;
    int s;
    ...
    if (!kstack) return 0;
    t = find_free_slot();
    if (!t) { kfree(kstack); return 0; }

    t->pid              = next_pid++;
    t->stack            = kstack;
    t->kernel_stack_top = (uint32_t)(kstack + PROC_STACK_SIZE);
    t->cr3              = current->cr3;      /* share address space */
    t->parent_pid       = current->pid;
    t->exit_code        = 0;
    t->state            = PROC_READY;
    t->pending_signals  = 0;
    t->is_thread        = 1;
    t->thread_group_id  = current->thread_group_id;

    for (s = 0; s < NSIG; s++) t->signal_handlers[s] = current->signal_handlers[s];
    for (s = 0; s < MAX_FDS; s++) t->fd_table[s] = current->fd_table[s];
    for (s = 0; s < MAX_REGIONS; s++) t->vm_regions[s] = current->vm_regions[s];

    /*
     * Build a ring-3 interrupt frame at the top of the kernel stack.
     * Layout (growing down):
     *   user_ss, user_esp, eflags, user_cs, user_eip,
     *   err_code(0), int_no(0),
     *   edi, esi, ebp, esp_dummy, ebx, edx, ecx, eax,
     *   ds, es, fs, gs
     */
    sp = (uint32_t *)(kstack + PROC_STACK_SIZE);
    *--sp = 0x23u;              /* user SS */
    ...
    *--sp = 0; *--sp = 0;      /* err_code, int_no */
    /* pusha order (edi,esi,ebp,esp_dummy,ebx,edx,ecx,eax) */
    *--sp = 0; *--sp = 0; *--sp = 0; *--sp = 0;
    *--sp = 0; *--sp = 0; *--sp = 0; *--sp = 0;
    /* ds, es, fs, gs */
    *--sp = 0x23u; *--sp = 0x23u; *--sp = 0x23u; *--sp = 0x23u;
